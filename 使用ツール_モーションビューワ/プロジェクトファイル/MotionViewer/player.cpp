...
void OperatePlayer(void)
{
	...
	const char* cMotionName;

	if (g_aPlayer.nState < 5)
	{
		cMotionName = MOTION_NAME[g_aPlayer.nState];
	}
	else
	{
		cMotionName = "その他モーション";
	}

	PrintDebugProc("[ 現在のモーション ( %s ) ] : %d\n", cMotionName, g_aPlayer.nState);
